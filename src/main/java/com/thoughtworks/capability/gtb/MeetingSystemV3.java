...
public class MeetingSystemV3 {
    ...
    public static void main(String[] args) {
        ...
        ZonedDateTime localMeetingTime = londonMeetingTime.withZoneSameInstant(ZoneId.systemDefault());
        ZonedDateTime now = ZonedDateTime.now();
        if (now.isAfter(localMeetingTime)) {
            ZonedDateTime tomorrow = localMeetingTime.plus(Period.ofDays(1));
            tomorrow = tomorrow.withZoneSameInstant(ZoneId.of("America/Chicago"));
            LocalDateTime nextMeetingTime = tomorrow.toLocalDateTime();
            String showTime = formatter.format(nextMeetingTime);
